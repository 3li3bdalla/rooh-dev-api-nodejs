...
router.use(function(req, res, next) {
  ...
  if (token) {
    ...
    jwt.verify(authToken[1], gRouter.get(Constants.SERVER.JWT_SECRET_KEY).toString(), async function(err, decoded) {
      if (err) {
        if(authToken[1]==Constants.SERVER.GUEST_TOKEN){
            ...
            res.status(401).json({status:0,message:RESPONSE_MESSAGES.STATUS_MSG.ERROR.ERROR_TOKEN_AUTH.customMessage});
            return;
        }
      } else {
        //DESCRYPTION
        ...
        Models.Users.findOne({"_id":decoded._id},{},function(err, result){
          if(!result || (result.accessToken=="") || (result.accessToken != authToken[1])) return res.status(401).json({status:0,message:RESPONSE_MESSAGES.STATUS_MSG.ERROR.ERROR_TOKEN_AUTH.customMessage});

          if(result.isBlocked == true){
              return res.status(403).json({status: 0, message: RESPONSE_MESSAGES.STATUS_MSG.ERROR.BLOCK_USER.customMessage});
          }else if(result.isDeleted == true){
              return res.status(403).json({ status: 0, message: RESPONSE_MESSAGES.STATUS_MSG.ERROR.NO_USER_EXISTS.customMessage});
          }else{
    ...
    res.status(403).json({status:0,message:RESPONSE_MESSAGES.STATUS_MSG.ERROR.EMPTY_TOKEN.customMessage});
    return;
  }
});
