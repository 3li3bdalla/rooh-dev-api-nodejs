...
async function checkAppointmentConflict(obj) {
    ...
    if (obj.type == "HOME") {
        ...
        returnObject = {
            ...
            "date": "",
            "slot": ""
        } //status 0 means date and slot is available for appointment
        return returnObject;
    } else { //ONLINE/ONSITE
...
async function checkAppointments(checkDate, checkSlots, doctorId) {
    ...
    let appntments = await Models.Appointment.find(criteria, {}, {
        lean: true
    });
            ...
            for (let slot of checkSlots) {
                ...
                if (apts.slots.indexOf(slot) != -1) {
                    returnObject = {
                        ...
                        "date": checkDate,
                        "slot": slot
                    } //status 1 means date and slot is not available for appointment
                    return returnObject;
                }
            }
        }
...
async function checkWeeklyAppointments(checkDate, checkSlots, doctorId) {
    ...
    let datesList;
    let criteria = {
        "homeService.weeklyDates.startDate": {
            $gte: checkDate
        },
        "homeService.weeklyDates.endDate": {
            $lt: checkDate
        },
    ...
    for (let date of dates) {
        ...
        if (data.professional.workingHours[dow].working == true) {
            ...
            if (response == true) {
                return {
                    "status": 0,
                    "startTime": data.professional.workingHours[dow].startTime,
                    "endTime": data.professional.workingHours[dow].endTime,
                    "day": moment().day(dow).format('dddd')
                }
            }
        }
    }
...
module.exports = {
        ...
        try {
            ...
            if (aptStatus.status == 1) { // return unavailable date and slot
                ...
                return sendResponse.sendSuccessData({
                    ...
                    "date": aptStatus.date,
                    "slot": aptStatus.slot
                }, APP_CONSTANTS.STATUSCODE.SUCCESS, req.headers.language, RESPONSE_MESSAGES.STATUS_MSG.SUCCESS.SUCCESS, res);
            }
            ...
            if (!payload.checkAvailability) {
                ...
                if (payload.appointmentId != undefined && payload.appointmentId != "") { // checking single date in case of updation
                    ...
                    apptmentDetails = await Models.Appointment.findOneAndUpdate({
                        "_id": ObjectId(payload.appointmentId)
                    }, dataToUpdate)
                ...
                let userSettingsNew = await Models.Users.findOne({
                    "_id": ObjectId(recId)
                }, {
                    deviceType: 1,
                    deviceToken: 1,
    ...
    get: async (req, res, next) => {
        ...
        try {
            ...
            if (query.type == "1") {
                finalResponse = {
                    "upcoming": upComingAppointments.appointment,
                    count: upComingAppointments.count
                }
            } else if (query.type == "2") {
                finalResponse = {
                    "past": pastAppointments.appointment,
                    count: pastAppointments.count
                }
            } else {
                finalResponse = {
                    "upcoming": upComingAppointments.appointment,
            ...
            } // added upcoming appointments to "past" object just for app testing - asked by Rishi on 2Aug
    ...
    receivedAppointment: async (req, res, next) => {
        ...
        try {
            ...
            let checkDate = moment(new Date().toLocaleString("en-US", {
                timeZone: req.headers.timezone
            })).format('YYYY-MM-DD');
            if (query.bookingType && query.bookingType != "") {
                criteria.type = query.bookingType
            } // ONLINE,ONSITE,HOME
            ...
            if (query.lastId && query.lastId != "") {
                criteria._id = {
                    $lt: ObjectId(query.lastId)
                }
            } //FOR PAGINATION
            if (is.undefined(query.count) || is.empty(query.count)) {
                query.count = 100;
            } else {
    ...
    bookedProfessionalsList: async (req, res, next) => {
        ...
        try {
            ...
            let doctors = await Models.Appointment.find(criteria, {
                doctor: 1
            }, {
                lean: true
            }).distinct('doctor');
    ...
    getConsultantPriceList: async (req, res, next) => {
        try {
            ...
            let doctorData = await Models.Users.findOne({
                "_id": ObjectId(obj.doctorId)
            }, {
                "professional.professionalSpeciality": 1,
    ...
    getCommonService: async (req, res, next) => {
        try {
            ...
            if (payload.search) {
                let keyword = {
                    $regex: commonFunctions.escapeRegExp(payload.search),
                    $options: 'i'
                };
    ...
    createEReport: async (req, res, next) => {
        try {
            ...
            let payload = await UniversalFunctions.validateSchema(req.body, schema, {
                presence: "required"
            });
            ...
            let checkAppointment = await Dao.findOne(Models.Appointment, {
                "_id": ObjectId(payload.appointmentId)
            }, {
                "_id": 1,
                user: 1,
    ...
    ereportUpdate: async (req, res, next) => {
        try {
            ...
            let schema = Joi.object().keys({
                appointmentId: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.string().required().length(24),
                    otherwise: Joi.optional()
                }),
                dependentId: Joi.string().optional().length(24),
                allergies: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.string().optional().allow(''),
                    otherwise: Joi.optional().allow('')
                }),
                chiefComplaint: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.string().required(),
                    otherwise: Joi.optional()
                }),
                diagnostics: Joi.array().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.array().items(Joi.string().required().length(24)),
                    otherwise: Joi.optional()
                }),
                history: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.string().required(),
                    otherwise: Joi.optional()
                }),
                examinationDiagnosticsResult: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.string().required(),
                    otherwise: Joi.optional()
                }),
                managementPlanRecommendation: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_REPORT,
                    then: Joi.string().required(),
                    otherwise: Joi.optional()
                }),
                "reportId": Joi.string().optional().length(24),
                "tests": Joi.array().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.TEST_REQ,
                    then: Joi.array().items(Joi.string().required().length(24)),
                    otherwise: Joi.optional()
                }),
                "testComments": Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.TEST_REQ,
                    then: Joi.string().optional().allow(''),
                    otherwise: Joi.optional().allow('')
                }),
                "medications": Joi.array().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_PRESCRIPTION,
                ...
                medicationsComments: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.E_PRESCRIPTION,
                    then: Joi.string().optional().allow(''),
                    otherwise: Joi.optional().allow('')
                }),
                followUpDate: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.FOLLOW_UP_APPOINTMENT,
                    then: Joi.string().required(),
                    otherwise: Joi.optional().allow('')
                }),
                startTime: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.FOLLOW_UP_APPOINTMENT,
                    then: Joi.string().optional().allow(''),
                    otherwise: Joi.optional().allow('')
                }),
                endTime: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.FOLLOW_UP_APPOINTMENT,
                    then: Joi.string().optional().allow(''),
                    otherwise: Joi.optional().allow('')
                }),
                slots: Joi.array().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.FOLLOW_UP_APPOINTMENT,
                    then: Joi.array().optional(),
                    otherwise: Joi.optional()
                }),
                referDoctorId: Joi.string().when('stepwise', {
                    is: APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.PROFESSIONAL_FACILTY,
                    then: Joi.string().required().length(24),
                    otherwise: Joi.optional().allow('')
                }),
            ...
            if (payload.reportId) {
                ...
                let checkReport = await Dao.populateData(Models.PatientAppointmentReport, {
                ...
                if (payload.stepwise === APP_CONSTANTS.DATABASE.REPORTS_STEPWISE.SKIP) {
                    ...
                    var apptdata = await Dao.findAndUpdate(Models.Appointment, {
                        "_id": ObjectId(checkReport.appointmentId._id)
                    }, {
                        "isAlreadyReportGenerate": true,
                ...
                await Dao.findAndUpdate(Models.PatientAppointmentReport, {
                    "_id": ObjectId(payload.reportId)
                }, payload, {
                    lean: true
                });
            ...
            let [appointmentData] = await Promise.all([
                ...
                Dao.findAndUpdate(Models.Appointment, {
                    "_id": ObjectId(payload.appointmentId)
                }, {
                    "reportType": 2
                }, {
    ...
    getReports: async (req, res, next) => {
        ...
        try {
            ...
            if (payload.appointmentId) {
                criteria = {
                    appointmentId: ObjectId(payload.appointmentId)
                };
    ...
    getProfessionalAndFacilityList: async (req, res, next) => {
        ...
        try {
            ...
            if (payload.type === APP_CONSTANTS.DATABASE.USER_ROLES.PROFESSIONAL) {
                ...
                criteria = {
                    status: {
                        $nin: [APP_CONSTANTS.DATABASE.APPOINTMENT_STATUS.COMPLETED]
                    },
                    doctor: ObjectId(userData._id)
                }
    ...
    getMedicalReportsDiagnostics: async (req, res, next) => {
        let diagnosis = await Models.PatientAppointmentReport.find({
    ...
    getMedicalReports: async (req, res, next) => {
        ...
        try {
            ...
            let checkFolderId = await Dao.findOne(Models.Folder, {
                "_id": ObjectId(payload.folderId)
            }, {
                _id: 1,
                folderName: 1
            }, {
            ...
            switch (checkFolderId.folderName) {
                ...
                case APP_CONSTANTS.DATABASE.FOLDERS.REPORTS['en']: {
                    ...
                    if (payload.type === APP_CONSTANTS.DATABASE.REPORT_TYPE.SCANED) {
                        ...
                        let [data1, count1] = await Promise.all([
                            Dao.getData(Models.File, criteria, {}, {
                                lean: true,
                                sort: {
                                    createdAt: -1
                                }
                        ...
                        if (payload.diagnosticsId) {
                            criteria.diagnostics = {
                                $in: [ObjectId(payload.diagnosticsId)]
                            };
                        }
                ...
                case APP_CONSTANTS.DATABASE.FOLDERS.LABS['en']: {
                    if (payload.type === APP_CONSTANTS.DATABASE.REPORT_TYPE.SCANED) {
                        ...
                        // criteria.userId = ObjectId(req.credentials._id);
                        criteria.folderId = {
                            $ne: APP_CONSTANTS.DATABASE.FOLDERS.RADIOLOGY['en']
                        };
    ...
    addFeedback: async (req, res, next) => {
        try {
            ...
            let userRole = await Models.Users.findOne({
            ...
            if (userRole != null) {
                if ((userRole.role).indexOf("PROFESSIONAL") != -1) {
                    ...
                    var feedbackData = await Models.ApointmentFeedback.findOneAndUpdate({
                        "beneficiaryId": ObjectId(userData._id),
    ...
    feedbacksList: async (req, res, next) => {
        try {
            ...
            if (is.undefined(payload.count) || is.empty(payload.count)) {
                payload.count = 100;
            } else {
        ...
        try {
            ...
            if (paymentDetails.result.code == "000.000.000" || paymentDetails.result.code == "000.100.110" || paymentDetails.result.code == "000.100.111" || paymentDetails.result.code == "000.100.112" || paymentDetails.result.code == "000.200.100") {
                if (paymentDetails.registrationId) {
                    ...
                    console.log("/////////////////////")
                    var resp = await Models.Users.updateOne({
                        "_id": ObjectId(userData._id)
                    }, {
                        $addToSet: {
                            paymentRegistrationIds: paymentDetails.registrationId
                        }
                    });
    ...
    manageWallet: async (req, res, next) => {
        try {
            ...
            if (payload.action == "1") {
                ...
                let checkBalance = await Models.Users.findOne({
                    "_id": ObjectId(userData._id)
                }, {
                    "wallet.balance": 1,
                    "_id": 0
            ...
            Models.Users.updateOne({
                "_id": ObjectId(userData._id)
            }, dataToSet, async function (updateErr, updateCheck) {
                if (updateErr) {
                    ...
                    if (payload.action == "1") {
                        ...
                        paymentData = await savePaymentDetails({}, {
                            "type": "2",
                            "amount": payload.amount,
                            "paymentMode": "Wallet"
                        }, walletSuccessMessage.message.en, walletSuccessMessage, payload.action);

                    } else if (payload.action == "0") {
                        if (payload.type == "1") {
                            var apptdata = await Models.Appointment.findOne({
    ...
    callPayment: async (req, res, next) => {
        try {
            ...
            if (payload.paymentMode == "wallet") {
                ...
                let paymentDetails = await Models.Users.updateOne({
                    "_id": ObjectId(userData._id)
                }, dataToSet);
            ...
            Models.Payments.aggregate([{
                ...
                if (err) {
                    ...
                    return sendResponse.sendSuccessData({
                        ...
                        totalAmount: (totalAmount.length > 0 && totalAmount[0].totalAmount) ? totalAmount[0].totalAmount : 0
                    }, 200, req.headers.language, RESPONSE_MESSAGES.STATUS_MSG.SUCCESS.DEFAULT, res);
                }
            });
        } catch (err) {
    ...
    removeCard: async (req, res, next) => {
        try {
            ...
            var payload = await UniversalFunctions.validateSchema(req.body, schema, {
                presence: "required"
            });
            ...
            if (paymentDetails.result.code == "000.000.000" || paymentDetails.result.code == "000.100.110" || paymentDetails.result.code == "000.100.111" || paymentDetails.result.code == "000.100.112") {
                Models.Users.updateOne({
                    ...
                    $pull: {
                        paymentRegistrationIds: payload.registrationId
                    }
                });
                return sendResponse.sendSuccessData({}, APP_CONSTANTS.STATUSCODE.SUCCESS, req.headers.language, RESPONSE_MESSAGES.STATUS_MSG.SUCCESS.DEFAULT, res);
            } else {
            ...
            if (paymentDetails.result.code == "000.000.000" || paymentDetails.result.code == "000.100.110" || paymentDetails.result.code == "000.100.111" || paymentDetails.result.code == "000.100.112") {
                ...
                if (payload.createRegistration && paymentData.registrationId) {
                    await Models.Users.updateOne({
                        ...
                        $addToSet: {
                            paymentRegistrationIds: paymentData.registrationId
                        }
                    });
                }
    ...
    addCSFContract: async (req, res, next) => {
        try {
            ...
            let checkTemplate = await Dao.findOne(Models.CommonServiceType, {
                ...
                "type": APP_CONSTANTS.DATABASE.COMMON_SERVICE_TYPE.CSFCONTRACT
            }, {}, {
                lean: true
            });
            ...
            let checkContract = await Dao.findOne(Models.Contract, {
                "professionalId": req.credentials._id,
                "contractExpireDate": {
                    $gt: (new Date().getTime())
                }
...
async function savePaymentDetails(payment, payload, paymentStatusMsg, languageMessages, action) {
    ...
    if (payload.type == "1") {
        var professionalData = await Models.Appointment.findOne({
            "_id": ObjectId(payload.id)
        }, {
            doctor: 1,
            _id: 0
...
function folderNames(folderNames) {
    ...
    let output = [];
    for (let x of folderNames) {
        output.push(x.folderName['en'])
    }
    return output;
}
